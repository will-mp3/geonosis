...
from model_lists import dec2025
...
Environment Variables:
  ...
  OUTPUT_BASE   Download directory (default: /Volumes/robots/models)
    ...
    if args.list_categories:
        ...
        for cat, name in sorted(dec2025.CATEGORIES.items()):
            models = dec2025.get_models_by_category(cat)
            total_size = sum(m["size_gb"] for m in models)
    ...
    if args.list_sizes:
        print("\nSize Categories:")
        for size, name in sorted(dec2025.SIZE_CATEGORIES.items()):
            models = dec2025.get_models_by_size(size)
            total_size = sum(m["size_gb"] for m in models)
    ...
    if args.all:
        selected_models = dec2025.get_all_models()
        print(f"Selected: ALL models ({len(selected_models)} total)")

    elif args.category:
        selected_models = dec2025.get_models_by_category(args.category)
        ...
        selected_models = dec2025.get_models_by_size(args.size)
        if not selected_models:
        ...
        # Find models by name
        all_models = dec2025.get_all_models()
